...
if "CMSSW_VERSION" in os.environ.keys():
    from VariableHelpers import variable
    from HistogramHelpers import Count
...
def doROCandWP(setup):
    ...
    ########################################
    input_filename = "results/TMVA/TMVA_{0}.root".format(setup.name)
    ...
    for method_and_options in setup.li_methods:
        ...
        if setup.draw_roc:
            ...
            expression_and_index = []
            for var in varlist:
                expression_and_index.append([str(var.attributes['Expression'].value),
                                             int(var.attributes['VarIndex'].value)])

            expression_and_index.sort(key = lambda x:x[1])
            expressions = [x[0] for x in expression_and_index]
        ...
        if setup.draw_wps:
            ...
            for wp in setup.working_points:
                ...
                #print "Test 4.1!"
                print "weights/{0}.weights.xml".format(wp["file_name"])
                ...
                other_filename = "results/TMVA/TMVA_{0}.dat".format("_".join(wp["file_name"].split("_")[:-1]))                    
                ...
                print "Signal Efficiency: {0}%".format(eff_total_sig)
                print "Background Rejection: {0}%".format(1-eff_total_bkg)
                print "-----\n\n"
...
def plotROCs(name, li_setups, extra_text = ""):
    ...
    print len(li_line_styles)

    #print "Test!"

    outputs = []
    for setup in li_setups:
        outputs.append(doROCandWP(setup))
    ...
    for view in ["left_top", "all"]:
        ...
        #output_filename = "{0}_{1}.png".format(name, view)
        #print output_filename
        #c.Print(output_filename)
        #c.Print("{0}_{1}.pdf".format(name, view))
        c.Print("{0}_{1}.root".format(name, view))
# end of plotROCs
